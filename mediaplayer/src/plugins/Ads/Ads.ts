...
class Ads {
  ...
  static getInstance() {
    // Checamos si ya tenemos una instancia, 
    // si no la hay la vamos a crear
    if(!Ads.instance) {
  ...
  private initAds() {
    // Aquí vamos a inicializar a los ads.
    // Con ... tomamos todos los elementos que están 
    // en ALL_ADS y los dejamos 'caer' sobre un nuevo arreglo
    this.ads = [...ALL_ADS];
  }

  // Expondremos una forma en la que 
  // Ads le dara ads a quien los pida
  getAd() {
    // Si nuestros ads se llegan a acabar los vamos a volver a inicializar
    if(this.ads.length === 0) {
      this.initAds();
    }
    // Pop va a sacar un valor de 
    // este arreglo y lo va a regresar
    return this.ads.pop();
  }
}
